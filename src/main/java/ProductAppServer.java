...
class ProductAppProcessThread extends Thread
{
	private ProductAppServer Server;
	private String address;
	private ProductJob job;
	public ProductAppProcessThread(ProductAppServer server,GridJob j,String add)
	{
		Server=server;
		job=(ProductJob)j;
		address=add;
...
public class ProductAppServer extends UnicastRemoteObject implements JobInterface
{
 ...
 private Hashtable results = new Hashtable();
	...
	public static void main(String args[])
	{
		try
		{
			...
			JobInterface server = new ProductAppServer();
	...
	public ProductAppServer() throws RemoteException
	{
	}
	synchronized public void submitJob(GridJob job)
	{
		ProductAppProcessThread thread = new ProductAppProcessThread(this,job,myAddress);
		thread.start();
	}
